...
const ControlPanel = ({
  ...
  useEffect(() => {
    ...
    clearPendingPreset();
  }, [pendingPreset, clearActiveSavedPreset, setColorStops, updateControl, setSelectedPreset, clearPendingPreset]);
    ...
    <>
      <div className="mt-8 mb-4 flex items-center justify-end gap-2">
        <div className={`${isColorsLocked ? "opacity-50 pointer-events-none" : ""} flex items-center gap-2`}>
          <button onClick={onReverseColors} disabled={isColorsLocked} className={`h-8 w-8 rounded-lg transition-colors flex items-center justify-center ${isColorsLocked ? "bg-surface text-muted-foreground/50 cursor-not-allowed" : "bg-surface hover:bg-surface-hover text-surface-foreground"}`} title="Reverse colors"><span className="text-[13px]">↔</span></button>
          <button onClick={onSavePreset} disabled={isSaveDisabled} className={`h-8 w-8 rounded-lg transition-colors flex items-center justify-center ${isSaveDisabled ? "bg-surface text-muted-foreground/50 cursor-not-allowed" : "bg-surface hover:bg-surface-hover text-surface-foreground"}`} title="Save preset"><Save size={14} /></button>
          <button onClick={onDeletePreset} disabled={isDeleteDisabled} className={`h-8 w-8 rounded-lg transition-colors flex items-center justify-center ${isDeleteDisabled ? "bg-surface text-muted-foreground/50 cursor-not-allowed" : "bg-surface hover:bg-surface-hover text-surface-foreground"}`} title="Delete selected preset"><Trash size={14} /></button>
        </div>
        <div className="w-px h-5 bg-border" />
        <button onClick={() => { clearActiveSavedPreset(); onReset(); }} className="h-8 px-3 rounded-lg bg-surface-hover hover:bg-surface-active text-foreground text-[10px] uppercase tracking-wider transition-colors flex items-center gap-1.5">
          <RotateCcw size={12} /> Reset
        </button>
      </div>
      <div className="mt-0 p-4 rounded-xl text-xs" style={{ background: "hsl(var(--panel-bg))", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", border: "1px solid hsl(var(--panel-border))", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.15)" }}>
        <button type="button" onClick={() => setIsMotionOpen((o) => !o)} className="w-full flex items-center justify-between mb-2 group">
          <div className="py-1 text-[14px] uppercase tracking-wider text-foreground/80">Motion</div>
          <ChevronDown size={16} className="text-muted-foreground group-hover:text-foreground transition-all duration-300" style={{ transform: isMotionOpen ? "rotate(180deg)" : "rotate(0deg)" }} />
        </button>
        <div style={{ display: "grid", gridTemplateRows: isMotionOpen ? "1fr" : "0fr", transition: "grid-template-rows 350ms cubic-bezier(0.4, 0, 0.2, 1)" }}>
            ...
            <div className="grid items-start gap-4 sm:grid-cols-2 lg:grid-cols-3 pb-1">
              ...
              <div className="w-full min-w-0 flex flex-col gap-1">
                ...
                <div className="flex flex-col gap-1 items-start justify-start h-fit">
                  ...
                  <ControlSlider label="Duration" value={controls.explosionDurationMs} onChange={(v) => updateControlAndClearSaved("explosionDurationMs", v)} min={150} max={2000} step={50} formatValue={(v) => `${Math.round(v)}ms`} />
                </div>
              </div>
              <div className="w-full min-w-0 flex flex-col gap-1">
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-[10px] uppercase tracking-wider text-muted-foreground">Reorg</label>
                  <button onClick={() => { const next = controls.disableReorg < 0.5 ? 1 : 0; updateControlAndClearSaved("disableReorg", next); }} className={`relative w-8 h-[18px] rounded-full transition-colors ${controls.disableReorg < 0.5 ? "bg-muted-foreground" : "bg-muted"}`} title={controls.disableReorg < 0.5 ? "Reorg enabled" : "Reorg disabled"}>
                    <span className="absolute top-[2px] h-[14px] w-[14px] rounded-full bg-white transition-all" style={{ left: controls.disableReorg < 0.5 ? 14 : 2 }} />
                  </button>
                </div>
                <div className={`flex flex-col gap-1 items-start justify-start h-fit transition-opacity ${controls.disableReorg >= 0.5 ? "opacity-30 pointer-events-none" : ""}`}>
                  ...
                  <ControlSlider label="Float ms" value={controls.floatDurationMs} onChange={(v) => updateControlAndClearSaved("floatDurationMs", v)} min={200} max={2000} step={50} formatValue={(v) => `${Math.round(v)}ms`} />
...
const Typography = () => {
  ...
  const colorPresetGroups = useMemo(() => {
    ...
    return groups
      .map((group) => ({ ...group, presets: group.presets.filter((preset) => !hidden.has(preset.name)) }))
      .filter((group) => group.presets.length > 0);
  }, [deletedColorPresetNames]);

  useEffect(() => { savePresetsToStorage(savedPresets); }, [savedPresets]);
  ...
  useEffect(() => {
    ...
    setSavedPresets((prev) => {
      ...
      const next = prev.map((preset) => {
        ...
        if (uniqueName !== existingName) { changed = true; return { ...preset, name: uniqueName }; }
        return preset;
      });
  ...
  useEffect(() => {
    try { window.localStorage.setItem(DELETED_COLOR_PRESETS_STORAGE_KEY, JSON.stringify(deletedColorPresetNames)); } catch { /* ignore */ }
  }, [deletedColorPresetNames]);
  ...
  const handleSavePreset = () => {
    ...
    setSavedPresets((prev) => {
      ...
      const existing = new Set(prev.map((p) => (p.name?.trim() || "").toLowerCase()).filter(Boolean));
      const dedupedName = ensureUniqueName(baseName, existing);
      const newPreset: BannerPreset = { id, name: dedupedName, createdAt: Date.now(), controls: bannerControls ? { ...bannerControls } : ({} as Controls), colorStops: [...normalizedStops] };
  ...
  const handleDeleteSelectedPreset = () => {
    if (activeSavedPresetId) {
      ...
      if (!nextSelected) { setActiveSavedPresetId(null); return; }
    ...
    if (!builtIn) return;
    const group = colorPresetGroups.find((g) => g.name === builtIn.group.name);
    if (!group) return;
    const idx = group.presets.findIndex((p) => p.name === selectedPreset);
    if (idx === -1) return;
    setDeletedColorPresetNames((prev) => prev.includes(selectedPreset) ? prev : [...prev, selectedPreset]);
    const nextInGroup = group.presets[idx + 1] ?? group.presets[idx - 1] ?? null;
    if (nextInGroup) { applyBuiltInPreset(nextInGroup.name, nextInGroup.stops); return; }
    const groupIndex = colorPresetGroups.findIndex((g) => g.name === builtIn.group.name);
    const nextGroup = colorPresetGroups[groupIndex + 1];
    if (nextGroup && nextGroup.presets.length > 0) { applyBuiltInPreset(nextGroup.presets[0].name, nextGroup.presets[0].stops); return; }
    const prevGroup = colorPresetGroups[groupIndex - 1];
    if (prevGroup && prevGroup.presets.length > 0) { const p = prevGroup.presets[prevGroup.presets.length - 1]; applyBuiltInPreset(p.name, p.stops); return; }
    setSelectedPreset("Custom");
  };
    ...
    <div className="min-h-screen bg-transparent flex flex-col items-center justify-center px-6 pt-16 pb-16">
      ...
      <div className="w-full max-w-[1312px]">
        <InteractiveTypeBanner
          ...
          onResetComplete={() => { setHasInteracted(false); setOpenPickerIndex(null); }}
          renderControls={(props) => (
            <ControlPanel
              ...
              onSyncBannerState={(controls, updateControl) => { setBannerControls(controls); setBannerUpdateControl(() => updateControl); }}
              onSyncReset={(resetFn) => setBannerReset(() => resetFn)}
              onReverseColors={() => { setActiveSavedPresetId(null); setColorStops([...normalizedStops].reverse()); setSelectedPreset("Custom"); }}
              ...
              isDeleteDisabled={hasInteracted || (activeSavedPresetId === null && (selectedPreset === "Custom" || !findBuiltInPreset(selectedPreset))) || (activeSavedPresetId !== null && savedPresets.length === 0)}
            />
          )}
        />
        <div
        ...
        >
          <div className="flex flex-col gap-3 py-2">
            ...
            <div style={{ display: "grid", gridTemplateRows: isColorsOpen ? "1fr" : "0fr", transition: "grid-template-rows 350ms cubic-bezier(0.4, 0, 0.2, 1)" }}>
              <div style={{ overflow: "hidden" }}>
                <div className="relative">
                  <div className={`transition-all duration-300 ${hasInteracted ? "pointer-events-none" : ""}`} style={hasInteracted ? { filter: "blur(4px)", opacity: 0.4 } : undefined}>
                    <div className="flex flex-col gap-3">
                      <div className="flex flex-wrap items-center gap-3 pt-1 pb-3">
                        {normalizedStops.map((stop, index) => (
                          <Popover key={`color-stop-${index}`} open={openPickerIndex === index} onOpenChange={(open) => setOpenPickerIndex(open ? index : null)}>
                            <PopoverTrigger asChild>
                              <button type="button" className="flex items-center gap-2 rounded-md border border-border bg-transparent px-2 py-1 hover:border-foreground/30">
                                <span className="h-6 w-6 rounded border border-border" style={{ backgroundColor: stop }} />
                                <span className="text-[10px] text-muted-foreground">{stop}</span>
                              </button>
                            </PopoverTrigger>
                            <PopoverContent align="start" sideOffset={8} className="w-72 p-3">
                              <div onPointerDown={(event) => event.stopPropagation()} onPointerMove={(event) => event.stopPropagation()} onPointerUp={(event) => event.stopPropagation()}>
                                <div className="mb-2 flex items-center justify-between">
                                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Picker</span>
                                  <button type="button" onClick={() => setOpenPickerIndex(null)} className="px-2 py-1 rounded-md bg-surface hover:bg-surface-hover text-surface-foreground text-[10px] uppercase tracking-wider transition-colors">Done</button>
                                </div>
                                <ColorPickerGradient value={stop} idSuffix={`type-color-${index}`} onChange={(nextColor) => { setActiveSavedPresetId(null); const next = [...normalizedStops]; next[index] = nextColor; setColorStops(next); setSelectedPreset("Custom"); }} />
                              </div>
                            </PopoverContent>
                          </Popover>
                        ))}
                        ...
                        <div className="flex flex-col gap-1.5">
                          ...
                          <div className="grid gap-2 p-1" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(100px, 1fr))" }}>
                            {savedPresets.map((preset) => {
                              ...
                              return (
                                <button key={preset.id} onClick={() => applySavedPreset(preset)} title={preset.name || "Saved preset"} className={`group rounded-lg overflow-hidden transition-all ${isSelected ? "ring-2 ring-foreground/60 ring-offset-1 ring-offset-background" : "ring-1 ring-foreground/10 hover:ring-foreground/30"}`}>
                                  <div className="flex w-full h-7">{(preset.colorStops || []).map((c, i) => (<div key={`${preset.id}-s-${i}`} className="flex-1 h-full" style={{ backgroundColor: c }} />))}</div>
                                  <div className="px-1.5 py-1 bg-card/80 text-[9px] text-muted-foreground group-hover:text-foreground truncate text-center uppercase tracking-wider transition-colors">{preset.name || "Saved"}</div>
                                </button>
                              );
                            })}
                          </div>
                        ...
                        <div key={group.name} className="flex flex-col gap-1.5">
                          ...
                          <div className="grid gap-2 p-1" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(100px, 1fr))" }}>
                            {group.presets.map((preset) => {
                              ...
                              return (
                                <button key={preset.name} onClick={() => { setActiveSavedPresetId(null); setColorStops(preset.stops); setSelectedPreset(preset.name); }} className={`group rounded-lg overflow-hidden transition-all ${isSelected ? "ring-2 ring-foreground/60 ring-offset-1 ring-offset-background" : "ring-1 ring-foreground/10 hover:ring-foreground/30"}`}>
                                  <div className="flex w-full h-7">{preset.stops.map((c, i) => (<div key={`${preset.name}-s-${i}`} className="flex-1 h-full" style={{ backgroundColor: c }} />))}</div>
                                  <div className="px-1.5 py-1 bg-card/80 text-[9px] text-muted-foreground group-hover:text-foreground truncate text-center uppercase tracking-wider transition-colors">{preset.name}</div>
                                </button>
                              );
                            })}
                          </div>
                  ...
                  {hasInteracted && (
                    <div className="absolute inset-0 flex items-center justify-center z-10">
                      <button onClick={() => { setActiveSavedPresetId(null); bannerReset?.(); }} className="px-4 py-2 rounded-full bg-surface/90 border border-foreground/20 text-foreground text-[11px] uppercase tracking-wider backdrop-blur-sm hover:bg-surface-hover/90 hover:border-foreground/40 transition-all shadow-lg">Reset to edit colors</button>
                    </div>
                  )}
                </div>
              </div>
